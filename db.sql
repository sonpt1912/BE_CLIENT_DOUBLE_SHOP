...
(
    ...
    PRIMARY KEY (`id`),
    UNIQUE INDEX `code_UNIQUE` (`code` ASC) VISIBLE,
    INDEX          `FK_PR_BRAND_idx` (`id_brand` ASC) VISIBLE,
    INDEX          `FK_PR_COLLAR_idx` (`id_collar` ASC) VISIBLE,
    INDEX          `FK_PR_MATERIAL_idx` (`id_material` ASC) VISIBLE,
    INDEX          `FK_PR_CATEGORY_idx` (`id_category` ASC) VISIBLE,
    CONSTRAINT `FK_PR_BRAND`
        FOREIGN KEY (`id_brand`)
            REFERENCES `double_shop`.`brand` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,
    CONSTRAINT `FK_PR_COLLAR`
